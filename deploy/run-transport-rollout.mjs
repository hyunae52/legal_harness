...
if (!key || host !== 'cta@136.67.179.84' || !/^\/home\/cta\/transport-[0-9a-f]{12}-packet\.json$/.test(packet) || !report) throw Error('Invalid fixed-target rollout arguments');
const events = [];
async function phase(name) {
  try {
    const event = await runRemotePhase(name, () => exec('ssh', ['-o', 'BatchMode=yes', '-o', 'IdentitiesOnly=yes', '-o', 'StrictHostKeyChecking=yes', '-i', key,
      host, 'python3', '/home/cta/transport-interview-rollout.py', name, packet], { timeout: 180000, windowsHide: true, maxBuffer: 1024 * 1024 }));
