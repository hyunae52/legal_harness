...
export const correctionInputs: Record<'prepare' | 'confirm' | 'status' | 'search', InputSchema> = {
  prepare: { type: 'object', additionalProperties: false,
    required: ['request_id', 'title', 'previous_claim', 'correction', 'why', 'sources', 'keywords', 'next_checks', 'public_safe'], properties: {
      client_session: publicSessionSchema, request_id: id, title: str(120), previous_claim: str(2000), correction: str(3000), why: str(2000),
  ...
  confirm: { type: 'object', additionalProperties: false, required: ['proposal_id', 'proposal_hash', 'confirmation_token', 'confirm'], properties: {
    client_session: publicSessionSchema, proposal_id: id, proposal_hash: { type: 'string', pattern: '^[a-f0-9]{64}$' }, confirmation_token: { type: 'string', pattern: '^[a-f0-9]{64}$' },
  ...
  status: { type: 'object', additionalProperties: false, required: ['proposal_id'], properties: { proposal_id: id, client_session: publicSessionSchema } },
  search: { type: 'object', additionalProperties: false, required: ['query'], properties: { query: str(20000) } },
};
const { client_session: _session, ...publicProposalProperties } = correctionInputs.prepare.properties;
const response = { description: 'Proposal or actual PR status. Uncertain states are not success.', content: { 'application/json': { schema: {
  type: 'object', properties: {
    client_session: publicSessionSchema, proposal_id: id, state: { type: 'string' }, proposal_hash: { type: 'string' }, confirmation_token: { type: 'string' },
    question: { type: 'string' }, public_preview: { type: 'object', properties: { schema_version: { type: 'integer' }, proposal: { type: 'object', properties: publicProposalProperties, additionalProperties: true } }, additionalProperties: true },
