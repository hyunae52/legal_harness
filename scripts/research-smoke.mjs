...
try {
  ...
  const report = { status: 'pass', transport: 'REST and MCP SSE', tool_count: tools.length,
...
} finally { await mcp.close(); if (runtime) await runtime.close(); if (server) { server.closeAllConnections(); await new Promise(r => server.close(r)); } }
