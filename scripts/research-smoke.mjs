...
if (!origin) {
  ...
  runtime = createApp({ env: publicMode ? { TAXLAB_PUBLIC_ACCESS: '1', TAXLAB_PUBLIC_SESSION_SECRET: randomBytes(32).toString('hex') }
    : { TAXLAB_API_KEY: key }, law: createLegalRetrievalClient(createKoreanLawClient()) });
...
try {
  assert.equal((await request('/api/research/start', {}, false)).status, publicMode ? 400 : 401);
  ...
  const session = publicMode ? { client_session: state.client_session } : {};
  if (publicMode) assert.ok(state.client_session);
  const lookup = await request('/api/research/retrieve', { ...session, research_id: state.research_id, expected_revision: state.revision,
  ...
  const input = { ...session, research_id: state.research_id, expected_revision: state.revision, expected_state_version: lookup.data.state_version,
  ...
  assert.equal((await request('/mcp', {}, false)).status, publicMode ? 400 : 401);
  ...
  const current = (await httpMcp.callTool({ name: 'get_legal_research', arguments: { ...session, research_id: state.research_id } })).structuredContent;
  ...
  const answered = (await httpMcp.callTool({ name: 'answer_legal_question', arguments: {
    ...session, research_id: state.research_id, expected_revision: current.revision, expected_state_version: current.state_version,
  ...
  const report = { status: 'pass', access_mode: publicMode ? 'public' : 'authenticated', transport: 'REST, MCP SSE and stateless Streamable HTTP', tool_count: tools.length,
