...
| 답변 의존 후보 상태: `unresolved` | 조건부ㆍ부분답변만 허용 | `false` | `false` |
| 모든 필수ㆍ유지 후보가 `supported/excluded` | 근거 있는 완료 허용 | `true` | `true` |

이 3개는 새 사례가 아니라 같은 `CRIT-01`에 stub 상태를 주입하는 변형이다.

여기서 `true/false`는 후보 상태가 아니라 `질문 범위 완료`와 `선언 범위 완료`의 값이다.
...
제8절 행동 게이트를 위한 내부 구현ㆍ테스트는 **잠정 workflow 회귀검수안**으로 진행할 수 있다. `CRIT-01~04`와 `CTRL-01~04`의 네 쌍에 대한 사람 법률승인은 그 결과를 법적 정답 검증, 사용자 대상 파일럿 또는 공개 배포 승인으로 승격하기 전에 필요하다.
